...
In addition to the staff translation diagram, provide a **fretboard chord and scale view**.

For a chord Root and Chord quality, it shows:
...
The same picker also includes every scale available in Notation view plus Chromatic. A scale map shows every matching tone through fret 5, with the tonic distinguished. One tone per string is selected for playback, initially the lowest available tone; selecting another tone replaces it, and swiping across the strings plays the selected tones. Root controls the tonic and note spelling for every scale, including Chromatic.
...
Defaults to G major and persists (including in the URL) independently of the Notation view's Key/Scale selection. Fretboard controls only appear when the Fretboard view is active; conversely, Key and Scale only appear when the Notation view is active.
