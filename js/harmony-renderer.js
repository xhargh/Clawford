...
function nodeMarkup(node, selectedId) {
  return `<button class="harmony-node quality-${node.quality}" data-harmony-node="${escape(node.id)}" aria-pressed="${node.id === selectedId}"><strong>${escape(node.numeral)}</strong><span>${escape(node.name)}</span><small>${escape(node.qualityLabel)}</small></button>`;
}
...
export function renderHarmonyOutput({ keyValue, mode, includeSevenths, selectedNodeId, selectedPair, activeView = "map", trail = [] }) {
  ...
  const edges = chords.flatMap((from) => chords.filter((to) => from.id !== to.id && (analyzeTransition(from, to, mode, includeSevenths).functionLabel || from.id === selectedNode.id || to.id === selectedNode.id)).map((to) => relationshipMarkup(from, to, mode, includeSevenths, pair))).join("");
  ...
  const body = activeView === "matrix" ? matrixMarkup(chords, mode, includeSevenths, pair) : `<div class="harmony-content"><section class="harmony-map-panel" aria-labelledby="harmony-map-title"><div class="section-heading"><div><p class="eyebrow">${activeView === "progression" ? "Possible destinations" : "Directed relationships"}</p><h3 id="harmony-map-title">${activeView === "progression" ? `From ${escape(selectedNode.name)}` : "Follow the arrows"}</h3></div><p class="harmony-help">Arrows are directional: D7 → G resolves differently from G → D7.</p></div><div class="harmony-node-map">${chords.map((chord) => nodeMarkup(chord, selectedNode.id)).join("")}</div><div class="harmony-edges" aria-label="Accessible relationship list">${edges}</div></section><aside class="harmony-detail-panel" aria-live="polite"><p class="eyebrow">Transition explanation</p>${explanationMarkup(source, destination, mode, includeSevenths)}<div class="harmony-trail"><div class="section-heading"><h3>Exploration trail</h3><div><button type="button" data-harmony-play${trail.length ? "" : " disabled"}>Play</button><button type="button" data-harmony-undo${trail.length ? "" : " disabled"}>Undo</button><button type="button" data-harmony-reset>Reset</button></div></div><div>${trailMarkup}</div></div></aside></div>`;
